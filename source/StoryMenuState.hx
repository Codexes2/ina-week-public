...
class StoryMenuState extends MusicBeatState
{
	...
	static function weekData():Array<Dynamic>
	{
		return [
			...
			['Botan', 'Lalion', "Poi"],
			['Weight of the Universe', "Unravelling Distortion", "Radiant Heaven", "Our Nonstop Story", "Bordering Final Story"]
		];
	}
	...
	{
		//weekUnlocked = unlockWeeks(); -- this is really weird for some reason. im manually opening the weeks instead
		...
		antiBonk = new FlxText(FlxG.width * 0.7, 600, 0, "", 32);
		antiBonk.setFormat("VCR OSD Mono", 18, FlxColor.WHITE, RIGHT);
		...
		txtTracklist.color = 0xFFFFFFFF; // 0xFFe55777
		add(txtTracklist);
		// add(rankText);
		add(scoreText);
		add(txtWeekTitle);
	...
	override function update(elapsed:Float)
	{
		...
		if (curWeek == 2)
		{
			antiBonk.text = "First song has copyright :(\n\nPress P for copyright-free\n\n";
			if (Main.antiBonk) antiBonk.text += "\n(ANTI-BONK active)\n";

			if (FlxG.keys.justPressed.P)
			{
				Main.antiBonk = !Main.antiBonk;
				if (Main.antiBonk) FlxG.sound.play(Paths.sound('cancelMenu'));
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		FlxTween.color(yellowBG, 0.1, yellowBG.color, weekColors[curWeek]); // not yellow anymore :foob:
