...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		//trace ("initSonglist length: " + initSonglist.length);
			...
			else
				{
					switch(data[0])
					{
						case 'Weight Of The Universe' | 'Unravelling Distortion' | 'Radiant Heaven' | 'Our Nonstop Story' 
							| 'Bordering Final Story' | 'Stand By Me' | 'Inapurgation':
							if(!FlxG.save.data.inaHard)
								{
									//trace('beat ina week first');
								}
							else
								{
									//trace('good job');
						...
						case 'Eldritch Phantasm':
							if(!FlxG.save.data.beatInapurgation)
								{
									//trace('beat inapurgation first');
								}
							else
								{
									//trace('oh wow you beat it');
									songs.push(meta);
								}
						case 'Inaposter':
							if(!FlxG.save.data.beatEldritchPhantasm)
								{
									//trace('beat eldritch phantasm first');
								}
							else
								{
									//trace('wowowowow you beat it');
									songs.push(meta);
								}
						case 'Eidolic Stained Heart':
							//trace(" :) ");
	...
	override function update(elapsed:Float)
	{
		...
		if (accepted)
		{
			...
			if(songs[curSelected].songName == 'Inapurgation')
				{
					curDifficulty = 2;
				}
			
			if(songs[curSelected].songName == 'Eldritch Phantasm')
				{
					trace('time to die lol');
					curDifficulty = 2;
				}
	...
	override function beatHit()
	{
		...
		if (FlxG.camera.zoom < 1.35 && songs[curSelected].songName.toLowerCase() == 'mythbuster' && curBeat >= 8)
			{
		...
		//Sum extra detail
		if (FlxG.camera.zoom < 1.35 && songs[curSelected].songName.toLowerCase() == 'mythbuster' && curBeat >= 168 && curBeat < 200)
			{
				FlxG.camera.zoom += 0.060;
						
			}

		if (FlxG.camera.zoom < 1.35 && songs[curSelected].songName.toLowerCase() == 'poi' && curBeat >= 8)
			{
				FlxG.camera.zoom += 0.030;
					
			}
		//Sum extra detail
		if (FlxG.camera.zoom < 1.35 && songs[curSelected].songName.toLowerCase() == 'poi' && curBeat >= 168 && curBeat < 200)
			{
				FlxG.camera.zoom += 0.060;
						
			}					
			
		if (FlxG.camera.zoom < 1.35 && songs[curSelected].songName.toLowerCase() == 'gangimari' && curBeat >= 8)
			{
				FlxG.camera.zoom += 0.030;
					
			}
		//Sum extra detail
		if (FlxG.camera.zoom < 1.35 && songs[curSelected].songName.toLowerCase() == 'gangimari' && curBeat >= 168 && curBeat < 200)
			{
				FlxG.camera.zoom += 0.060;
